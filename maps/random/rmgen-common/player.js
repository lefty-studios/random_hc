 ...
 */
function placeStartingEntities(location, playerID, civEntities, dist = 2, orientation = BUILDING_ORIENTATION)
{
	// Place the central structure
	let i = 0;
	let firstTemplate = civEntities[i].Template; 
	g_Map.placeEntityPassable("skirmish/structures/default_civil_centre", playerID, location, orientation);
	++i;
...
function placePlayerBaseBerries(args)
{
	...
	for (let tries = 0; tries < get("maxTries", 30); ++tries)
	{
		let position = new Vector2D(0, get("distance", 25)).rotate(randomAngle()).add(basePosition);
		if (createObjectGroup(
			new SimpleGroup(
...
function placePlayerBaseMines(args)
{
	...
	for (let tries = 0; tries < get("maxTries", 75); ++tries)
	{
		...
		for (let i = 0; i < mineCount; ++i)
		{
			...
			pos[i] = new Vector2D(0, get("distance", 30)).rotate(angle).add(basePosition).round();
